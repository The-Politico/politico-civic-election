...
class Race(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        **uid**: :code:`{office.uid}_{cycle.uid}_race`
        """
        self.uid = "{}_{}_race".format(self.office.uid, self.cycle.uid)

        name_label = "{0} {1}".format(self.cycle.name, self.office.label)

        if self.special:
            self.uid = "{}:special".format(self.uid)
            name_label = "{} Special".format(name_label)
